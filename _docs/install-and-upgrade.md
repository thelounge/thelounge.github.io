...
To learn how to configure The Lounge, go to [the configuration section](/docs/configuration). To learn how to add users, read [the users section](/docs/users). Keep in mind that when using the `thelounge` cli command, you need to run it as the `thelounge` user, e.g `sudo -u thelounge thelounge --help`.
