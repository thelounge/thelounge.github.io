...
A list of all available plugins can be found [on the npm registry](https://www.npmjs.com/search?q=keywords:thelounge-theme) and installed with `thelounge install`. For example, to install a plugin called `thelounge-plugin-closepms`, run:
